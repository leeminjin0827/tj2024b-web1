...
const onLogin = ( ) => {
	...
	// 2. INPUT 입력값 가져오기
	const mid = midinput.value; console.log( mid );
	const mpwd = mpwdinput.value; console.log( mpwd );
	// 3. 유효성검사
	...
	// * fetch 설정
	const option = {
		method : 'POST' , // 전송할 http method 선택
	...
	fetch(`/tj2024_web1/member/login` , option )
		...
		.then( data => {
			if( data > 0 ) {
				alert('로그인 성공');
				console.log( toastSocket );
				toastSocket.send( `${mid}님 접속`);
				location.href="../index.jsp"} // ../ 상위 폴더로 이동 뜻
			else{ alert('로그인 실패'); }
		})
