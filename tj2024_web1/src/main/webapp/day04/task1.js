...
const visitWrite = ( ) => {
	...
	fetch( `/tj2024_web1/day03/visit2` , option )
...
const visitFindAll = ( ) => {
	...
	let html = '';
		...
		// 2-2 fetch
		fetch( `/tj2024_web1/day03/visit2`)
			...
			.then( data => {
				...
				// 3. 출력 // .innerHTML : 저장한 마크업의 html문자열 속성
...
const visitUpdate = ( num ) => { // prompt()함수로 수정할 content/age 받기
	// 1. 수정할 식별자(num)
	// 2. fetch 이용한 서블릿에게 HTTP update METHOD 처리 요청
...
const visitDelete = ( num ) => {
	...
	fetch( `/tj2024_web1/day03/visit2?num=${ num }` , option )
		.then( r => r.json() ) // 응답 결과를 json타입으로 변환
		.then( data => {
			if( data == true ){ alert('삭제성공'); visitFindAll(); } // 만약에 응답이 true 이면
			else{ alert(`삭제실패`) }
		})
		.catch( e => { console.log( e ); } )
} // f end
