...
# [1] 게시물 전체 조회 , select * from 테이블명;
select * from board;
# [2] 게시물 테이블과 회원테이블 내부조인 조회 , 게시물의 회원번호를 이용하여 회원 아이디 조회 하기 위해서
# board b : 테이블명 별칭명 , 테이블이 길거나 복잡할때 간단한 이름으로 정의
	# board b : board테이블을 sql문법에 'b' 라는 별칭으로 정의 장점 : 코드 간소화
# [3] 카테고리 별 조회
select * from board b inner join member m on b.mno = m.mno where cno = 1; -- 1번 카테고리(뉴스) 게시물만 조회
select * from board b inner join member m on b.mno = m.mno where cno = 2; -- 2번 카테고리(이벤트) 게시물만 조회
# [4] 페이징 처리 조회
	# limit : 조회된 레코드 결과를 제한,
	# limit 개수 : 첫번째 레코드부터 개수만큼만 제한해서 조회
    # limit 시작인덱스 , 개수 : 시작인덱스 부터 개수만큼만 제한해서 조회
select * from board limit 2; -- 조회 결과에서 2개만 조회
select * from board limit 1 , 3; -- 조회 결과에서 1(두번째레코드)부터 3개 레코드만 조회
	# 컨셉 : 페이지별로 게시물을 5개씩 출력
	# 1페이지 : 0 ~ 4 , 2페이지 5 ~ 9 , 3페이지 10 ~ 14 , 4페이지 15 ~ 19 , 5페이지 20 ~ 24
...
select * from board limit 20 , 5;	# 5페이지
