...
// - @wwebServlet HTTP vs - @ServerEndpoint WS 
@ServerEndpoint("/chatsocket")
public class ServerSocket {
	
	// [*] 접속된 세션(접속성공한 클라이언트 소켓 정보) 저장 , list 컬렉션으로 여러개 Session 를 저장하기.
	...
	// [1] 클라이언트 소켓이 서버소켓에 접속을 햇을때 , onOpen
	@OnOpen // 서버 소켓에 클라이언트 소켓들이 접속 했을때 @onopen 이벤트
	...
	// [3] 클라이언트 소켓과 연결이 닫혔을때( 클라이언트소켓(객체)의 포함된 JS 가 새로고침(객체가 지워졌을때) ) 실행되는 함수
	@OnClose
