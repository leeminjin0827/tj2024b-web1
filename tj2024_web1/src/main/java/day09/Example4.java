...
public class Example4 extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// 1. 현재 서블릿의 로컬 경로 알기,( 즉 서버 경로 조회 )
			// (배포전)자바 코드 파일 경로 : C:\Users\82109\git\tj2024b-web1\tj2024_web1\src\main\java\day09\Example4.java
			// (배포후)자바 코드 실행 파일의 서버 경로 : C:\Users\82109\Desktop\tj2024b_web1\.metadata\.plugins\org.eclipse.wst.server.core\tmp0\wtpwebapps\tj2024_web1\
			// req.getSercletContext().getRealPath("경로") : 서버내 경로를 가져오는 함수.
		...
		factory.setSizeThreshold( 1024 * 1024 );	// 2. 업로드 용량 제한 설정	, 1바이ㅌ 기준 , 1024바이트 -> 1kb , 1024*1024 -> 1mb
			...
			if( formItems != null && !formItems.isEmpty() ) {
				...
				for( int index = 0; index <= formItems.size()-1 ; index++ ) {
					...
					if( fileItem.isFormField() ) { // 일반 폼 자료인지. 
						...
						System.out.println( uuid ); // - 253480ed-aade-471a-a70f-88f089c040e3ee
							// - UUID 와 업로드 파일명 연결 , 만약에 파일명에 '-'(하이픈) 포함되면 uuid 와 파일명 구분이 힘들다.
							// 만약에 사용자가 업로드한 파일명에 -(하이픈)를_(언더바)로 변경 하기. 이유 : -(파이픈)는 uuid 구분용
							// "문자열".replaceAll( "변경할문자" , "새로운문자" ); : 문자열내 변경할문자가 존재하면 새로운문자로 치환
							// 예시] 253480ed-aade-471a-a70f-88f089c040e3ee.png
